import { c as createKeyStorage, l as loadSettings, s as saveSettings, M as MSG, C as COMMON_LANGUAGES, A as APP_NAME, b as APP_TAGLINE } from './key-storage-BRtVc9A1.js';
...
function languageOptions(current) {
  const list = [...COMMON_LANGUAGES];
  if (current && !list.includes(current)) list.unshift(current);
  return list.map(
    (lang) => `<option value="${escape(lang)}"${lang === current ? " selected" : ""}>${escape(lang)}</option>`
  ).join("");
}
async function render() {
  ...
  const hasKey = provider ? Boolean(await keyStorage.getApiKey(provider.providerId)) : false;
  const configured = Boolean(provider && hasKey);
  const statusHtml = configured ? `<div class="status ok">
         <span class="dot"></span>
         <span class="info">
           <span class="label">Ready to use</span>
           <span class="sub">${escape(provider.displayName)} · ${escape(provider.model)}</span>
         </span>
       </div>` : `<div class="status warn">
         <span class="dot"></span>
         <span class="info">
  ...
  root.querySelector("#native")?.addEventListener("change", (e) => {
    settings.nativeLanguage = e.target.value;
    void saveSettings(settings).then(showSavedToast);
  });
  const result = root.querySelector("#result");
  root.querySelector("#test")?.addEventListener("click", async (e) => {
